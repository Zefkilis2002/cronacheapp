...
const BackgroundImage = memo(({ bgImage, updateItemPosition, setBackgroundImages, isSelected = false, onSelect = () => {}, showSelection = true }) => {
  ...
  return (
    <KonvaImage
      ...
      onClick={() => onSelect(bgImage.id)}
      onTap={() => onSelect(bgImage.id)}
      onDragEnd={(e) => {
        updateItemPosition(
          bgImage.id, 
...
function CanvasNews({
  ...
  // handler memoizzati per la selezione
  const handleSelectBackground = useCallback((id) => {
    setSelectedBackground(id);
    setSelectedLogo(null);
  }, [setSelectedBackground, setSelectedLogo]);

  const handleSelectLogo = useCallback((id) => {
    setSelectedLogo(id);
